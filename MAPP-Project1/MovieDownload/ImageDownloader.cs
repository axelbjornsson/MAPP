...
namespace MovieDownload
{
    ...
    public class ImageDownloader
    {
        ...
        public async Task DownloadImagesFromListOfMovieDTOs(List<MovieDTO> movieList)
        {
            foreach (var movie in movieList)
            {
                var localPosterPath = LocalPathForFilename(movie.imagePath);
                if (localPosterPath != String.Empty && !File.Exists(localPosterPath))
                {
                    await DownloadImage(movie.imagePath, localPosterPath, new CancellationToken());
                }
                movie.imagePath = localPosterPath;
            }
        }
    }
}